...
class NetworkDevice:
    def __init__(self, ip, mac, hostname="Unknown", os_type="Unknown", status="Unknown"):
        self.ip = ip
        self.mac = mac
...
class EnhancedDatabaseManager:
    ...
    def init_database(self):
        ...
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_metrics_device_time ON metrics(device_mac, timestamp)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_alerts_device ON alerts(device_mac)')
    ...
    def save_device(self, device):
        ...
        try:
            cursor.execute('''
                ...
                (mac, ip, hostname, os_type, status, monitoring_enabled, last_seen, hardware_info, services, username, password, bash_history)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                device.mac, device.ip, device.hostname, device.os_type, device.status,
                device.monitoring_enabled, device.last_update,
                json.dumps(device.hardware_info), json.dumps(device.services),
                device.username, device.password, device.bash_history
            ))
            conn.commit()
        finally:
    ...
    def save_metrics_batch(self, metrics_batch):
        ...
        try:
            cursor.executemany('''
                INSERT INTO metrics 
                (device_mac, timestamp, cpu_percent, memory_percent, disk_percent, 
    ...
    def load_devices(self):
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("SELECT mac, ip, hostname, os_type, status, monitoring_enabled, username, password, bash_history FROM devices")
            devices = {}
            for row in cursor.fetchall():
                mac, ip, hostname, os_type, status, monitoring_enabled, username, password, bash_history = row
                device = NetworkDevice(ip, mac, hostname, os_type, status)
                device.monitoring_enabled = bool(monitoring_enabled)
                device.username = username
                device.password = password
                device.bash_history = bash_history
                devices[mac] = device
            return devices
        finally:
            self.return_connection(conn)
            
    def save_snapshot(self, device_mac, name, description, data):
        """Save a snapshot to the database."""
        conn = self.get_connection()
        cursor = conn.cursor()
        try:
            cursor.execute('''
                INSERT INTO snapshots (device_mac, name, description, timestamp, data)
                VALUES (?, ?, ?, ?, ?)
            ''', (device_mac, name, description, datetime.now(), json.dumps(data)))
            conn.commit()
        finally:
            self.return_connection(conn)
    ...
    def load_snapshots(self):
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("SELECT s.name, s.description, d.hostname FROM snapshots s JOIN devices d ON s.device_mac = d.mac")
            return cursor.fetchall()
        finally:
            self.return_connection(conn)

class EnhancedSSHManager:
    """Enhanced SSH manager with connection pooling and sudo handling."""
    ...
    def connect_to_device(self, device, username, password, port=22):
        ...
        try:
            ...
            client.connect(device.ip, port=port, username=username, password=password, timeout=200)
    ...
    def execute_command(self, device, command, user_password=None):
        """
        Execute command on device, handling sudo non-interactively.
        `user_password` is the password for the currently connected user.
        """
        try:
            if not device.ssh_client or not device.ssh_client.get_transport().is_active():
                logger.warning(f"No active SSH connection to {device.ip} for command execution.")
                return None

            # If the command requires sudo, prepend the command to pipe the password.
            if command.strip().startswith('sudo') and user_password:
                # Use shlex to safely quote the password
                import shlex
                command_to_run = f"echo {shlex.quote(user_password)} | sudo -S -p '' {command.strip()[5:]}"
            else:
                command_to_run = command

            stdin, stdout, stderr = device.ssh_client.exec_command(command_to_run)
            output = stdout.read().decode('utf-8', errors='ignore')
            error = stderr.read().decode('utf-8', errors='ignore')

            # Filter out the common sudo password prompt warning
            if error and "sudo: a terminal is required" not in error:
                if "try again" not in error.lower():
                     logger.warning(f"Command error on {device.ip} for '{command}': {error.strip()}")

            return output
        except Exception as e:
            logger.error(f"Command execution failed on {device.ip}: {e}")
            device.ssh_client = None # Invalidate the connection
            return None

    def create_monitoring_account(self, device, initial_user_pass, new_username, new_password):
      #  """Create monitoring account on device and set up passwordless sudo."""
     try:
        # Detect the distribution to determine correct sudo group
        distro_check = self.execute_command(device, "cat /etc/os-release | grep -i '^ID='", user_password=initial_user_pass)
        
        # Determine sudo group based on distribution
        if distro_check and any(distro in distro_check.lower() for distro in ['fedora', 'rhel', 'centos', 'rocky', 'almalinux']):
            sudo_group = 'wheel'
        else:
            # Default to sudo group for Debian/Ubuntu systems
            sudo_group = 'sudo'
        
        # Commands to create user, set password, and grant sudo
        commands = [
            f"sudo useradd -m -s /bin/bash {new_username}",
            f"echo '{new_username}:{new_password}' | sudo chpasswd",
            f"sudo usermod -aG {sudo_group} {new_username}",
        ]
       
        # **BEST PRACTICE**: Add user to sudoers for passwordless operation
        # This is safer than editing sudoers directly. It creates a new file.
        sudoers_command = f"echo '{new_username} ALL=(ALL) NOPASSWD: ALL' | sudo tee /etc/sudoers.d/99-{new_username}"
       
        # Run all setup commands using the initial password
        all_commands = commands + [sudoers_command]
        for cmd in all_commands:
            result = self.execute_command(device, cmd, user_password=initial_user_pass)
            # Basic check for command failure
            if result is None:
                logger.error(f"A setup command failed on {device.ip}: {cmd}")
                return False
    ...
    def get_enhanced_metrics(self, device):
        ...
        try:
            ...
            cpu_output = self.execute_command(device, cpu_cmd, user_password=device.password)
            if cpu_output:
                try:
            ...
            mem_output = self.execute_command(device, mem_cmd, user_password=device.password)
            if mem_output:
                try:
            ...
            disk_output = self.execute_command(device, disk_cmd, user_password=device.password)
            if disk_output:
                try:
            ...
            load_output = self.execute_command(device, load_cmd, user_password=device.password)
            if load_output:
                try:
            ...
            proc_output = self.execute_command(device, proc_cmd, user_password=device.password)
            if proc_output:
                try:
            ...
            service_output = self.execute_command(device, service_cmd, user_password=device.password)
            services = {}
            if service_output:
    ...
    def get_bash_history(self, device):
    # """Get filtered bash history from all users in /home directory."""
      try:
        # Determine OS
        os_type_output = self.execute_command(device, "grep -oP '(?<=^ID=).+' /etc/os-release", user_password=device.password)
        # for Ubuntu this could be /etc/lsb-release
        os_type = os_type_output.strip().lower() if os_type_output else ""
        
        # Get current username
        current_user = self.execute_command(device, "whoami", user_password=device.password)
        current_user = current_user.strip() if current_user else ""
        
        # Find all .bash_history files in /home
        find_cmd = "find /home -name '.bash_history' -type f -readable 2>/dev/null"
        history_files = self.execute_command(device, find_cmd, user_password=device.password)
        
        if not history_files:
            return ""
        
        all_filtered_history = []
        install_command = ""
        
        # Determine package manager command based on OS
        ...
        for history_file in history_files.strip().split('\n'):
            if not history_file.strip():
                continue
                
            # Check if this is the current user's history file
            is_current_user = f"/home/{current_user}/.bash_history" == history_file.strip()
            
            # Read the history file
            cat_cmd = f"cat '{history_file.strip()}'"
            history_output = self.execute_command(device, cat_cmd, user_password=device.password)
            
            if not history_output:
                continue
            
            # Filter for install commands
            user_filtered_history = []
            username = history_file.split('/')[-2]  # Extract username from path
            
            if install_command:
                for line in history_output.splitlines():
                    if install_command in line:
                        # Add username context to the command
                        formatted_line = f"[{username}] {line.strip()}"
                        user_filtered_history.append(formatted_line)
            
            # Separate current user's history from others
            if is_current_user:
                current_user_history.extend(user_filtered_history)
            else:
                other_users_history.extend(user_filtered_history)
        
        # Combine results with current user's history first
        all_filtered_history = current_user_history + other_users_history
        
        return "\n".join(all_filtered_history)
        
      except Exception as e:
        logger.error(f"Failed to get bash history for {device.ip}: {e}")
        return ""
...
class ModernGUI:
    ...
    def apply_theme(self, theme_name):
        ...
        self.style.configure("TButton", background=theme["accent"], foreground=theme["select_fg"])
        self.style.configure("Treeview", background=theme["bg"], foreground=theme["fg"], fieldbackground=theme["bg"])
...
class EnhancedMonitoringThread:
    ...
    def _monitor_device(self, device):
        ...
        try:
            ...
            if not self.app.ssh_manager.connect_to_device(device, device.username, device.password):
                self.app.log_message(f"Monitoring: Could not connect to {device.ip}. Disabling monitoring.", "WARNING")
                device.monitoring_enabled = False
                self.app.db_manager.save_device(device) # Persist disabled state
                return None
...
class NetworkScanner:
    ...
    def get_local_network(self):
        ...
        try:
            ...
            ip_obj = ipaddress.IPv4Interface(f"{ipv4_info['addr']}/{netmask}")
    ...
    def scan_host(self, ip):
        ...
        if self.ping_host(ip):
            ...
            if mac:
                return NetworkDevice(ip, mac, hostname, os_type, "Online")
        return None
...
class EnhancedPCManagementApp:
    ...
    def __init__(self, root):
        ...
        self.devices = self.db_manager.load_devices()
    ...
    def detect_network(self):
        """Detect the current network and update the UI."""
        network = self.scanner.get_local_network()
        self.network_var.set(network)
        self.log_message(f"Detected network: {network}")
    ...
    def create_enhanced_toolbar(self):
        ...
        ttk.Label(middle_frame, text="🔍 Search:").pack(side=tk.LEFT, padx=(0, 5))
        ...
        # Network range
        ttk.Label(middle_frame, text="🌐 Network:").pack(side=tk.LEFT, padx=(20, 5))
        ...
        self.alerts_indicator = ttk.Label(right_frame, text="🔴 0 Alerts")
        self.alerts_indicator.pack(side=tk.RIGHT, padx=10)

        # Monitoring status
        self.monitoring_status = ttk.Label(right_frame, text="⏸️ Stopped")
    ...
    def create_enhanced_device_panel(self):
        ...
        columns = ('Status', 'IP', 'MAC', 'Hostname', 'OS', 'CPU %', 'Memory %', 'Last Update', 'Alerts')
        self.device_tree = ttk.Treeview(tree_frame, columns=columns, show='headings', height=20)

        # Configure columns with better widths
        column_widths = {'Status': 60, 'IP': 120, 'MAC': 150, 'Hostname': 150, 'OS': 100,
    ...
    def create_metrics_summary_cards(self, parent):
        ...
        self.total_devices_card, self.total_devices_label = self.modern_gui.create_metric_card(
            cards_frame, "Total Devices", "0", "", "#17a2b8"
        )

        self.monitored_devices_card, self.monitored_devices_label = self.modern_gui.create_metric_card(
            cards_frame, "Monitored", "0", "", "#28a745"
        )

        self.alerts_card, self.alerts_label = self.modern_gui.create_metric_card(
            cards_frame, "Active Alerts", "0", "", "#dc3545"
        )

        self.avg_cpu_card, self.avg_cpu_label = self.modern_gui.create_metric_card(
    ...
    def create_realtime_alerts_tab(self):
        ...
        ttk.Label(alert_controls, text="Filter:").pack(side=tk.LEFT, padx=(20, 5))
    ...
    def create_security_monitoring_tab(self):
        ...
        self.failed_logins_card, self.failed_logins_label = self.modern_gui.create_metric_card(
            security_summary, "Failed Logins (24h)", "0", "", "#dc3545"
        )

        self.suspicious_activity_card, self.suspicious_activity_label = self.modern_gui.create_metric_card(
    ...
    def create_deploy_tab(self):
        ...
        self.notebook.add(deploy_frame, text="🚀 Deploy")

        ttk.Label(deploy_frame, text="Username:").pack(pady=5)
        self.deploy_username_entry = ttk.Entry(deploy_frame, width=30)
        self.deploy_username_entry.pack(pady=5)

        ttk.Label(deploy_frame, text="Password:").pack(pady=5)
        self.deploy_password_entry = ttk.Entry(deploy_frame, show="*", width=30)
        self.deploy_password_entry.pack(pady=5)

        ttk.Button(deploy_frame, text="Save Credentials & Fetch History", command=self.deploy_to_selected).pack(pady=20)
    
    def create_history_tab(self):
        history_frame = ttk.Frame(self.notebook)
        self.notebook.add(history_frame, text="📜 History")
        
        self.history_text = scrolledtext.ScrolledText(history_frame, height=25, font=('Consolas', 10), state=tk.DISABLED)
        self.history_text.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        
    def create_snapshot_tab(self):
        snapshot_frame = ttk.Frame(self.notebook)
        self.notebook.add(snapshot_frame, text="📸 Snapshot")

        controls_frame = ttk.Frame(snapshot_frame)
        controls_frame.pack(fill=tk.X, padx=10, pady=10)

        ttk.Label(controls_frame, text="Snapshot Name:").pack(side=tk.LEFT, padx=5)
        self.snapshot_name_entry = ttk.Entry(controls_frame, width=30)
        self.snapshot_name_entry.pack(side=tk.LEFT, padx=5)

        ttk.Label(controls_frame, text="Description:").pack(side=tk.LEFT, padx=5)
        self.snapshot_desc_entry = ttk.Entry(controls_frame, width=50)
        self.snapshot_desc_entry.pack(side=tk.LEFT, padx=5)
        
        ttk.Button(controls_frame, text="Take Snapshot", command=self.take_snapshot).pack(side=tk.LEFT, padx=10)

        list_frame = ttk.LabelFrame(snapshot_frame, text="Saved Snapshots")
        list_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
        
        self.snapshot_tree = ttk.Treeview(list_frame, columns=('Name', 'Description', 'Host'), show='headings')
        self.snapshot_tree.heading('Name', text='Name')
        self.snapshot_tree.heading('Description', text='Description')
        self.snapshot_tree.heading('Host', text='Host')
        self.snapshot_tree.pack(fill=tk.BOTH, expand=True)
    ...
    def create_enhanced_status_bar(self):
        ...
        # Database status
        self.db_status = ttk.Label(indicators_frame, text="💾 DB: OK")
        self.db_status.pack(side=tk.RIGHT, padx=5)

        # Network status
        self.network_status = ttk.Label(indicators_frame, text="🌐 Net: Connected")
        self.network_status.pack(side=tk.RIGHT, padx=5)

        # Monitoring status
        self.monitoring_status_bar = ttk.Label(indicators_frame, text="📊 Monitor: Active")
        self.monitoring_status_bar.pack(side=tk.RIGHT, padx=5)

        # Time indicator
        self.time_label = ttk.Label(indicators_frame, text="")
        ...
        def quick_scan_thread():
            ...
            with ThreadPoolExecutor(max_workers=20) as executor:
                ...
                for future in as_completed(future_to_ip):
                    ip = future_to_ip[future]
                    try:
                        device = next((d for d in self.devices.values() if d.ip == ip), None)
                        if device:
                            if future.result():
    ...
    def scan_network(self):
        ...
        def scan_thread():
            ...
            for device in devices:
                ...
                else: # Update existing device info
                    self.devices[device.mac].ip = device.ip
                    self.devices[device.mac].hostname = device.hostname
                    self.devices[device.mac].os_type = device.os_type
                    self.devices[device.mac].status = device.status
                
                self.db_manager.save_device(self.devices[device.mac])
                    ...
                    for line_num, line in enumerate(lines, 1):
                        ...
                        try:
                            ...
                            if len(parts) >= 2:
                                ip = parts[0]
                                mac = parts[1]
                                hostname = parts[2] if len(parts) > 2 else "Imported"
                                ...
                                try:
                                    ...
                                    self.log_message(f"Invalid IP or MAC on line {line_num}: {ip}, {mac}")
    ...
    def setup_selected_devices(self):
        ...
        for item in selected:
            ...
            mac = device_data['values'][2]
            selected_macs.append(mac)

        if selected_macs:
            self.show_device_setup_dialog(selected_macs)

    def show_device_setup_dialog(self, device_macs):
        """Show setup dialog for selected devices"""
        dialog = tk.Toplevel(self.root)
        dialog.title(f"Setup {len(device_macs)} Device(s)")
        dialog.geometry("400x400")
        dialog.transient(self.root)
        dialog.grab_set()

        ttk.Label(dialog, text=f"Setup monitoring for {len(device_macs)} selected device(s)").pack(pady=10)

        # Credentials form
        ttk.Label(dialog, text="Initial/Root Username:").pack(pady=5)
        root_user_var = tk.StringVar(value="root")
        ttk.Entry(dialog, textvariable=root_user_var, width=30).pack(pady=2)

        ttk.Label(dialog, text="Initial/Root Password:").pack(pady=5)
        root_pass_var = tk.StringVar()
        ttk.Entry(dialog, textvariable=root_pass_var, show="*", width=30).pack(pady=2)

        ttk.Label(dialog, text="Monitoring Username (to create):").pack(pady=5)
        mon_user_var = tk.StringVar(value="netmonitor")
        ttk.Entry(dialog, textvariable=mon_user_var, width=30).pack(pady=2)
        
        ttk.Label(dialog, text="Monitoring User Password (to set):").pack(pady=5)
        mon_pass_var = tk.StringVar()
        ttk.Entry(dialog, textvariable=mon_pass_var, show="*", width=30).pack(pady=2)

        def start_setup():
            if not mon_pass_var.get():
                messagebox.showerror("Input Error", "The new monitoring user must have a password.", parent=dialog)
                return
            try:
                self.perform_bulk_setup(device_macs, root_user_var.get(),
                                      root_pass_var.get(), mon_user_var.get(), mon_pass_var.get())
                dialog.destroy()
            except Exception as e:
    ...
    def perform_bulk_setup(self, device_macs, root_user, root_pass, mon_user, mon_pass):
        """Perform bulk setup on selected devices"""
        def setup_thread():
            success_count = 0
            total_count = len(device_macs)
            ...
            for i, mac in enumerate(device_macs):
                ...
                try:
                    self.update_status(f"Setting up device {i+1}/{total_count} ({device.ip})")
                    ...
                    if self.ssh_manager.connect_to_device(device, root_user, root_pass):
                        ...
                        creation_success = self.ssh_manager.create_monitoring_account(
                            device, root_pass, mon_user, mon_pass)
                        ...
                        time.sleep(1) # Pause to prevent triggering fail2ban

                        if creation_success:
                            # **VERIFICATION STEP**
                            self.log_message(f"Verifying new account '{mon_user}' on {device.ip}...")
                            if self.ssh_manager.connect_to_device(device, mon_user, mon_pass):
                                self.log_message(f"Verification successful for {mon_user} on {device.ip}.")
                                device.username = mon_user
                                device.password = mon_pass
                                device.monitoring_enabled = True
                                self.db_manager.save_device(device)
                                success_count += 1
                                self.log_message(f"Successfully set up and verified monitoring on {device.ip}")
                                if device.ssh_client:
                                    device.ssh_client.close()
                                    device.ssh_client = None
                            else:
                                self.log_message(f"Account for '{mon_user}' was created on {device.ip}, but login verification failed. Check remote logs.", "ERROR")
                        else:
                            self.log_message(f"Failed to create monitoring account on {device.ip}. Check initial user's sudo rights.", "ERROR")
                    else:
                        self.log_message(f"Failed to connect to {device.ip} with initial credentials for user '{root_user}'.", "ERROR")
                except Exception as e:
                    self.log_message(f"An exception occurred during setup for {device.ip}: {str(e)}", "ERROR")
                finally:
    ...
    def start_monitoring_selected(self):
        ...
        for item in selected:
            ...
            if device and device.username and device.password:
                if self.ssh_manager.connect_to_device(device, device.username, device.password):
                    device.monitoring_enabled = True
                    self.db_manager.save_device(device)
                    count += 1
                else:
                    self.log_message(f"Could not connect to {device.ip} to start monitoring. Check credentials.")
            else:
                self.log_message(f"No credentials for {device.ip}. Please run setup or use the Deploy tab.")
    ...
    def remove_selected_devices(self):
        ...
        if messagebox.askyesno("Confirm Removal", f"Remove {len(selected)} selected device(s)? This is permanent."):
            removed_count = 0
            for item in selected:
                device_data = self.device_tree.item(item)
                mac = device_data['values'][2]
                ...
                if mac in self.devices:
                    ...
                    try:
                        ...
                        self.log_message(f"Error removing device with MAC {mac} from database: {e}")
    ...
    def show_device_context_menu(self, event):
        ...
        if item_id:
            ...
            context_menu.add_command(label="Rename Host", command=self.rename_selected_host)
    ...
    def rename_selected_host(self):
        ...
        if device:
            new_name = simpledialog.askstring("Rename Host", "Enter new name:", initialvalue=device.hostname)
            if new_name:
                device.hostname = new_name
                self.db_manager.save_device(device)
                self.update_device_display()
                self.log_message(f"Renamed host {device.ip} to {new_name}")
    ...
    def update_device_display(self):
        ...
        if self.device_tree.selection():
            ...
            selected_mac = self.device_tree.item(selected_item)['values'][2]
        ...
        try:
            sorted_devices = sorted(self.devices.values(), key=lambda d: ipaddress.IPv4Address(d.ip))
        except ipaddress.AddressValueError:
            sorted_devices = list(self.devices.values()) # Fallback if IPs are invalid

        for device in sorted_devices:
            # Apply search filter
            search_term = self.search_var.get().lower()
            if search_term and not (search_term in device.ip.lower() or 
                                    search_term in device.hostname.lower() or
            ...
            if device.last_update:
                ...
                if time_diff.total_seconds() < 60:
                    last_update = f"{int(time_diff.total_seconds())}s ago"
                elif time_diff.total_seconds() < 3600:
                    last_update = f"{int(time_diff.total_seconds()//60)}m ago"
                else:
                    last_update = f"{int(time_diff.total_seconds()//3600)}h ago"
    ...
    def update_summary_cards(self):
        ...
        # Update card labels
        if hasattr(self, 'total_devices_label'):
            self.total_devices_label.config(text=f"{total_devices}")
            self.monitored_devices_label.config(text=f"{monitored_devices}")
            self.alerts_label.config(text=f"{active_alerts}")
            self.avg_cpu_label.config(text=f"{avg_cpu:.1f}%")
    ...
    def update_alerts_display(self):
        ...
        filter_level = self.alert_filter_var.get().lower()

        for alert in sorted(active_alerts, key=lambda a: a.timestamp, reverse=True):
            if filter_level != "all" and alert.level != filter_level:
                continue
            ...
            # FIX: Handle non-numeric thresholds
            threshold_text = f"{alert.threshold:.1f}" if isinstance(alert.threshold, (int, float)) else str(alert.threshold)
        ...
        if critical_count > 0:
            self.alerts_indicator.config(text=f"🔴 {critical_count} Critical", foreground='red')
        elif alert_count > 0:
            self.alerts_indicator.config(text=f"🟡 {alert_count} Alerts", foreground='orange')
        else:
            self.alerts_indicator.config(text="✅ No Alerts", foreground='green')
    ...
    def update_monitoring_animation(self, frame):
        ...
        selected = self.device_tree.selection()
        # Clear plots if nothing is selected
        if not selected:
            self.cpu_ax.clear()
            ...
            self.cpu_ax.set_title("No device selected", fontsize=12)
            self.style_monitoring_plots()
    ...
    def create_circular_gauge(self, ax, value, label, color):
        ...
        # Value arc
        value_theta = np.linspace(0, 2*np.pi * (value/100), int(value * 2))
    ...
    def bulk_setup_dialog(self):
        ...
        ttk.Label(creds_frame, text="Initial/Root Username:").pack(pady=5)
        root_user_var = tk.StringVar(value="root")
        ttk.Entry(creds_frame, textvariable=root_user_var, width=30).pack(pady=2)

        ttk.Label(creds_frame, text="Initial/Root Password:").pack(pady=5)
        root_pass_var = tk.StringVar()
        ttk.Entry(creds_frame, textvariable=root_pass_var, show="*", width=30).pack(pady=2)

        ttk.Label(creds_frame, text="Monitoring Username (to create):").pack(pady=5)
        mon_user_var = tk.StringVar(value="netmonitor")
        ttk.Entry(creds_frame, textvariable=mon_user_var, width=30).pack(pady=2)
        
        ttk.Label(creds_frame, text="Monitoring User Password (to set):").pack(pady=5)
        mon_pass_var = tk.StringVar()
        ttk.Entry(creds_frame, textvariable=mon_pass_var, show="*", width=30).pack(pady=2)
        ...
        devices_inner.bind("<Configure>", on_configure)
        ...
        def start_bulk_setup():
            if not mon_pass_var.get():
                messagebox.showerror("Input Error", "The new monitoring user must have a password.", parent=dialog)
                return
            selected_devices = [mac for mac, var in device_vars.items() if var.get()]
            if not selected_devices:
                messagebox.showwarning("Warning", "Please select at least one device.", parent=dialog)
                return
    ...
    def export_report(self):
        ...
        filename = filedialog.asksaveasfilename(
            ...
            filetypes=[("HTML files", "*.html"),("JSON files", "*.json"), ("Text files", "*.txt")]
        )
    ...
    def on_device_select(self, event):
        ...
        if selected:
            ...
            if device:
                ...
                self.update_history_tab(device)
                # Auto-fill deploy tab
                self.deploy_username_entry.delete(0, tk.END)
                if device.username:
                    self.deploy_username_entry.insert(0, device.username)
                self.deploy_password_entry.delete(0, tk.END)
                if device.password:
                    self.deploy_password_entry.insert(0, device.password)
    ...
    def filter_devices_by_status(self, status):
        ...
        # This functionality is now implicitly handled by update_device_display and search
        # To implement a dedicated filter, you would add a state variable here
        self.update_device_display()
    ...
    def deploy_to_selected(self):
        """Deploy to the selected host."""
        selected = self.device_tree.selection()
        if not selected:
            messagebox.showwarning("Deploy Error", "No host selected.")
            return
        ...
        username = self.deploy_username_entry.get()
        password = self.deploy_password_entry.get()

        if not username or not password:
            messagebox.showwarning("Deploy Error", "Username and password are required.")
            return
        ...
        def deploy_thread():
            if self.ssh_manager.connect_to_device(device, username, password):
                history = self.ssh_manager.get_bash_history(device)
                device.bash_history = history
                self.db_manager.save_device(device)
                self.log_message(f"Successfully deployed to {device.ip} and fetched bash history.")
                self.root.after(0, lambda: self.update_history_tab(device))
                device.ssh_client.close()
                device.ssh_client = None
            else:
                self.log_message(f"Failed to deploy to {device.ip}.")

        threading.Thread(target=deploy_thread, daemon=True).start()

    def update_history_tab(self, device):
        """Update the history tab with the device's bash history."""
        self.history_text.config(state=tk.NORMAL)
        self.history_text.delete(1.0, tk.END)
        self.history_text.insert(tk.END, device.bash_history or "No history available.")
        self.history_text.config(state=tk.DISABLED)

    def take_snapshot(self):
        """Take a snapshot of the selected host."""
        selected = self.device_tree.selection()
        if not selected:
            messagebox.showwarning("Snapshot Error", "No host selected.")
            return
            
        name = self.snapshot_name_entry.get()
        description = self.snapshot_desc_entry.get()
        
        if not name:
            messagebox.showwarning("Snapshot Error", "Snapshot name is required.")
            return
        ...
        if not (device.monitoring_enabled and device.ssh_client) and not (device.username and device.password):
            messagebox.showwarning("Snapshot Error", "Host is not connected. Enable monitoring or deploy first.")
            return
            
        def snapshot_thread():
            # Ensure connection
            is_connected = device.ssh_client and device.ssh_client.get_transport().is_active()
            if not is_connected:
                is_connected = self.ssh_manager.connect_to_device(device, device.username, device.password)

            if not is_connected:
                self.log_message(f"Could not connect to {device.hostname} to take snapshot.")
                return

            snapshot_data = self.ssh_manager.get_enhanced_metrics(device)
            if snapshot_data:
                # FIX: Convert datetime object to a string before saving
                if 'timestamp' in snapshot_data and isinstance(snapshot_data['timestamp'], datetime):
                    snapshot_data['timestamp'] = snapshot_data['timestamp'].isoformat()
                
                self.db_manager.save_snapshot(device.mac, name, description, snapshot_data)
                self.log_message(f"Snapshot '{name}' taken for {device.hostname}")
                self.root.after(0, self.load_snapshot_list)
            else:
                self.log_message(f"Failed to take snapshot for {device.hostname}")

        threading.Thread(target=snapshot_thread, daemon=True).start()

    def load_snapshot_list(self):
        """Load the list of saved snapshots."""
        for item in self.snapshot_tree.get_children():
            self.snapshot_tree.delete(item)
            
        snapshots = self.db_manager.load_snapshots()
        for snap in snapshots:
            self.snapshot_tree.insert('', tk.END, values=snap)
...
def main():
    ...
    required_modules = ['paramiko', 'matplotlib', 'numpy', 'psutil', 'netifaces', 'getmac']
    missing_modules = []
    ...
    try:
        ...
        try:
            if 'app' in locals() and app.monitoring_thread.running:
